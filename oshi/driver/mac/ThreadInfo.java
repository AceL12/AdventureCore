...
public final class ThreadInfo {
   ...
   public static List<ThreadInfo.ThreadStats> queryTaskThreads(int pid) {
      String pidStr = " " + pid + " ";
      List<ThreadInfo.ThreadStats> taskThreads = new ArrayList<>();
      List<String> psThread = ExecutingCommand.runNative("ps -awwxM").stream().filter(s -> s.contains(pidStr)).collect(Collectors.toList());
      int tid = 0;
      ...
      for (String thread : psThread) {
         ...
         if (m.matches() && pid == ParseUtil.parseIntOrDefault(m.group(1), -1)) {
            ...
            taskThreads.add(new ThreadInfo.ThreadStats(tid++, cpu, state, sTime, uTime, pri));
         }
      }
