...
public class WindowsFileSystem extends AbstractFileSystem {
   ...
   public List<OSFileStore> getFileStores(boolean localOnly) {
      ArrayList<OSFileStore> result = getLocalVolumes(null);
            ...
            try {
               while (true) {
                  ...
                  if (!strMount.isEmpty() && (volumeToMatch == null || volumeToMatch.equals(volume))) {
                     ...
                     fs.add(
                        new WindowsOSFileStore(
                           String.format("%s (%s)", strName, strMount),
                           volume,
                           strName,
                           strMount,
                           options.toString(),
                           uuid,
                           "",
                           getDriveType(strMount),
                           strFsType,
                           systemFreeBytes.getValue(),
                           userFreeBytes.getValue(),
                           totalBytes.getValue(),
                           0L,
                           0L
                        )
   ...
   static List<OSFileStore> getWmiVolumes(String nameToMatch, boolean localOnly) {
      ...
      for (int i = 0; i < drives.getResultCount(); i++) {
         ...
         fs.add(
            new WindowsOSFileStore(
               String.format("%s (%s)", description, name),
               volume,
               label,
               name + "\\",
               options,
               ...
               "",
               getDriveType(name),
               WmiUtil.getString(drives, Win32LogicalDisk.LogicalDiskProperty.FILESYSTEM, i),
               free,
               free,
               total,
               0L,
