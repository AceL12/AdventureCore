...
final class LinuxFirmware extends AbstractFirmware {
   ...
   private final Supplier<LinuxFirmware.VcGenCmdStrings> vcGenCmd = Memoizer.memoize(LinuxFirmware::queryVcGenCmd);
   ...
   private String queryManufacturer() {
      String result = null;
      return (result = Sysfs.queryBiosVendor()) == null && (result = this.vcGenCmd.get().manufacturer) == null ? "unknown" : result;
   }

   private String queryDescription() {
      String result = null;
      return (result = Sysfs.queryBiosDescription()) == null && (result = this.vcGenCmd.get().description) == null ? "unknown" : result;
   }

   private String queryVersion() {
      String result = null;
      return (result = Sysfs.queryBiosVersion(this.biosNameRev.get().getB())) == null && (result = this.vcGenCmd.get().version) == null ? "unknown" : result;
   }

   private String queryReleaseDate() {
      String result = null;
      return (result = Sysfs.queryBiosReleaseDate()) == null && (result = this.vcGenCmd.get().releaseDate) == null ? "unknown" : result;
   }

   private String queryName() {
      String result = null;
      return (result = this.biosNameRev.get().getA()) == null && (result = this.vcGenCmd.get().name) == null ? "unknown" : result;
   }
   ...
   private static LinuxFirmware.VcGenCmdStrings queryVcGenCmd() {
      ...
      if (vcgencmd.size() >= 3) {
         ...
         return new LinuxFirmware.VcGenCmdStrings(vcReleaseDate, vcManufacturer, vcVersion, "RPi", "Bootloader");
      } else {
         return new LinuxFirmware.VcGenCmdStrings(null, null, null, null, null);
      }
   }
