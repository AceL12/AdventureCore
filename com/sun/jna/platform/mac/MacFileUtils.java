...
public class MacFileUtils extends FileUtils {
   ...
   public void moveToTrash(File... files) throws IOException {
      ...
      for (File src : files) {
         ...
         int status = MacFileUtils.FileManager.INSTANCE.FSPathMakeRefWithOptions(src.getAbsolutePath(), 1, fsref, null);
         if (status != 0) {
            failed.add(src + " (FSRef: " + status + ")");
         } else {
            status = MacFileUtils.FileManager.INSTANCE.FSMoveObjectToTrashSync(fsref, null, 0);
            if (status != 0) {
   ...
   public interface FileManager extends Library {
      MacFileUtils.FileManager INSTANCE = Native.load("CoreServices", MacFileUtils.FileManager.class);
