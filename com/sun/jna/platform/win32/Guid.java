   ...
   public static class GUID extends Structure {
      ...
      public static Guid.GUID fromBinary(byte[] data) {
         if (data.length != 16) {
            ...
            long data1Temp = data[0] & 255;
            data1Temp <<= 8;
            data1Temp |= data[1] & 255;
            data1Temp <<= 8;
            data1Temp |= data[2] & 255;
            data1Temp <<= 8;
            data1Temp |= data[3] & 255;
      ...
      public static Guid.GUID fromString(String guid) {
         ...
         if (guid.length() > 38) {
            ...
            for (int ix = 0; ix < 32; ix += 2) {
               bdata[ix / 2] = (byte)((Character.digit(_cnewguid[ix], 16) << 4) + Character.digit(_cnewguid[ix + 1], 16) & 0xFF);
            }

            if (bdata.length != 16) {
               throw new IllegalArgumentException("Invalid data length: " + bdata.length);
            } else {
               long data1Temp = bdata[0] & 255;
               data1Temp <<= 8;
               data1Temp |= bdata[1] & 255;
               data1Temp <<= 8;
               data1Temp |= bdata[2] & 255;
               data1Temp <<= 8;
               data1Temp |= bdata[3] & 255;
               newGuid.Data1 = (int)data1Temp;
