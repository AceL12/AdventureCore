...
public interface WinNT extends WinError, WinDef, WinBase, BaseTSD {
   ...
   public abstract static class ACCESS_ACEStructure extends WinNT.ACE_HEADER {
      ...
      public void read() {
         ...
         int sizeOfSID = super.AceSize - super.fieldOffset("SidStart");
         if (sizeOfSID > 0) {
            this.psid = new WinNT.PSID(this.getPointer().getByteArray(offsetOfSID, sizeOfSID));
         } else {
   ...
   public static final class LocaleMacros {
      ...
      public static final WinDef.LCID MAKELCID(int lgid, int srtid) {
         return new WinDef.LCID(_MAKELCID(lgid, srtid));
      }

      public static final WinDef.LCID MAKESORTLCID(int lgid, int srtid, int ver) {
         return new WinDef.LCID(_MAKELCID(lgid, srtid) | ver << 20);
      }
   ...
   public static class SECURITY_DESCRIPTOR_RELATIVE extends Structure {
      ...
      private final void setMembers() {
         this.read();
         if (this.Dacl != 0) {
            this.DACL = new WinNT.ACL(this.getPointer().share(this.Dacl));
         }

         if (this.Sacl != 0) {
            this.SACL = new WinNT.ACL(this.getPointer().share(this.Sacl));
         }

         if (this.Group != 0) {
            this.GROUP = new WinNT.PSID(this.getPointer().share(this.Group));
         }

         if (this.Owner != 0) {
            this.OWNER = new WinNT.PSID(this.getPointer().share(this.Owner));
         }
      }
   ...
   public abstract static class SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX extends Structure {
      ...
      public static WinNT.SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX fromPointer(Pointer memory) {
         ...
         WinNT.SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX result;
         switch (relationship) {
            case 0:
            case 3:
            case 5:
