...
public abstract class WevtapiUtil {
   ...
   public static String EvtFormatMessage(
      Winevt.EVT_HANDLE publisherMetadata, Winevt.EVT_HANDLE event, int messageId, int valueCount, Winevt.EVT_VARIANT[] values, int flags
   ) {
   ...
   public static Winevt.EVT_VARIANT EvtGetChannelConfigProperty(Winevt.EVT_HANDLE channelHandle, int propertyId) {
      ...
      if (!result && errorCode != 122) {
         ...
         result = Wevtapi.INSTANCE
            .EvtGetChannelConfigProperty(channelHandle, propertyId, 0, (int)propertyValueBuffer.size(), propertyValueBuffer, propertyValueBufferUsed);
         if (!result) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
         } else {
      ...
      if (!result && errorCode != 122) {
         ...
         result = Wevtapi.INSTANCE
            .EvtGetPublisherMetadataProperty(
               ...
               PropertyId,
               Flags,
               (int)publisherMetadataPropertyBuffer.size(),
