...
public class W32FileMonitor extends FileMonitor {
   ...
   private void handleChanges(W32FileMonitor.FileInfo finfo) throws IOException {
      ...
      } else if (!klib.ReadDirectoryChangesW(
            finfo.handle, finfo.info, finfo.info.size(), finfo.recursive, finfo.notifyMask, finfo.infoLength, finfo.overlapped, null
         )
         && !this.disposing) {
   ...
   protected synchronized void watch(File file, int eventMask, boolean recursive) throws IOException {
      ...
      if (dir == null) {
         ...
         if (WinBase.INVALID_HANDLE_VALUE.equals(handle)) {
            ...
            if (WinBase.INVALID_HANDLE_VALUE.equals(this.port)) {
            ...
            } else if (!klib.ReadDirectoryChangesW(handle, finfo.info, finfo.info.size(), recursive, notifyMask, finfo.infoLength, finfo.overlapped, null)) {
               int err = klib.GetLastError();
               throw new IOException(
                  "ReadDirectoryChangesW failed on "
                     + finfo.file
                     + ", handle "
                     + handle
                     + ": '"
               ...
               if (this.watcher == null) {
                  this.watcher = new Thread("W32 File Monitor-" + watcherThreadID++) {
                     ...
                     public void run() {
                        while (true) {
                           W32FileMonitor.FileInfo finfox = W32FileMonitor.this.waitForChange();
                           if (finfox == null) {
                              ...
                              try {
                                 W32FileMonitor.this.handleChanges(finfox);
   ...
   protected synchronized void unwatch(File file) {
      W32FileMonitor.FileInfo finfo = this.fileMap.remove(file);
      if (finfo != null) {
