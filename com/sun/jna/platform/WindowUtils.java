...
public class WindowUtils {
         ...
         this.whenDisplayable(w, new Runnable() {
            ...
            public void run() {
               try {
                  ...
                  Method setAlpha = peer.getClass().getMethod("setAlpha", float.class);
      ...
      protected void setMask(Component w, Raster raster) {
         ...
         if (region != null) {
            ...
            try {
               RasterRangesUtils.outputOccupiedRanges(raster, new RasterRangesUtils.RangesOutput() {
                  @Override
                  public boolean outputRange(int x, int y, int wx, int h) {
                     GDI32 gdix = GDI32.INSTANCE;
                     gdix.SetRectRgn(tempRgn, x, y, x + wx, y + h);
                     return gdix.CombineRgn(region, region, tempRgn, 2) != 0;
                  }
               });
            } finally {
      ...
      public BufferedImage getWindowIcon(WinDef.HWND hwnd) {
         ...
         if (result.intValue() == 0) {
            ...
            if (iconSize.width != 0 && iconSize.height != 0) {
               ...
               Pointer lpBitsColorPtr = new Memory(lpBitsColor.length);
               byte[] lpBitsMask = new byte[width * height * 24 / 8];
               Pointer lpBitsMaskPtr = new Memory(lpBitsMask.length);
               ...
               if (h == null || h.equals(WinBase.INVALID_HANDLE_VALUE)) {
                  ...
                  while (true) {
                     try {
                        ...
                        if (Kernel32.INSTANCE.FindNextVolume(h, volumeUUID, 50)) {
                           continue;
                        }
   ...
   private static class X11WindowUtils extends WindowUtils.NativeWindowUtils {
            ...
            if (dpy == null) {
               ...
               try {
                  ...
                  if (info != null) {
                     ...
                     for (int ix = 0; ix < this.alphaVisualIDs.length; ix++) {
                        this.alphaVisualIDs[ix] = list.get(ix).longValue();
                     }
      ...
      private static X11.Window getDrawable(Component w) {
         ...
         return id == 0 ? null : new X11.Window(id);
      }
      ...
      private class X11TransparentContentPane extends WindowUtils.NativeWindowUtils.TransparentContentPane {
         ...
         protected void paintDirect(BufferedImage buf, Rectangle bounds) {
            ...
            offset.x = offset.x + bounds.x;
            offset.y = offset.y + bounds.y;
