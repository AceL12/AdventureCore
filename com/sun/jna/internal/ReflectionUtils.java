...
public class ReflectionUtils {
   ...
   private static final Method METHOD_IS_DEFAULT = lookupMethod(Method.class, "isDefault");
   ...
   private static Constructor lookupDeclaredConstructor(Class clazz, Class... arguments) {
      if (clazz == null) {
         LOG.log(Level.FINE, "Failed to lookup method: <init>#{1}({2})", new Object[]{clazz, Arrays.toString((Object[])arguments)});
         return null;
      } else {
         try {
   ...
   private static Method lookupMethod(Class clazz, String methodName, Class... arguments) {
      if (clazz == null) {
         LOG.log(Level.FINE, "Failed to lookup method: {0}#{1}({2})", new Object[]{clazz, methodName, Arrays.toString((Object[])arguments)});
         return null;
      } else {
         try {
   ...
   private static Class lookupClass(String name) {
      try {
         ...
         LOG.log(Level.FINE, "Failed to lookup class: " + name, (Throwable)var2);
         return null;
      }
   }
   ...
   public static Object getMethodHandle(Method method) throws Exception {
      ...
      try {
         ...
         return mhViaFindSpecial(lookup, method);
      } catch (Exception var5) {
         Object lookupx = getConstructorLookupClass().newInstance(method.getDeclaringClass());
         return mhViaUnreflectSpecial(lookupx, method);
      }
   }
